...
public class MosaicGA {
    ...
    public static void main(String[] args) {
        if (args.length != 6) {
            System.err.println("Usage: java MosaicGA.java <input file> <populasi> <Generasi> <crossover rate> <mutationRate> <output file>");
            System.exit(1);
        }
        ...
        try {
            ...
            System.err.println("Error: format angka salah. Populasi & Generasi harus integer, crossover rate & mutationRate harus double.");
            return;
        }
        ...
        try {
            ...
            writeGenotypeToFile(best, outputFile);
...
// Hasil: 2 anak (child1, child2)
    static Individual[] rectangleCrossover(Individual parent1, Individual parent2, Random rng) {
        ...
        for (int r = r1; r <= r2; r++) {
            for (int c = c1; c <= c2; c++) {
                ...
                // OPTIONAL: kalau kamu mau sel clue tidak ikut berubah (tetap putih)
    ...
    static void writeGenotypeToFile(Individual ind, String filename) throws Exception {
        PrintWriter writer = new PrintWriter(new FileWriter(filename));
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                writer.print(ind.gene[i][j]);
                if (j < cols - 1) {
                    writer.print(" ");
                }
            }
        ...
        writer.close();
        System.out.println("Genotype telah disimpan ke file: " + filename);
    }
}
